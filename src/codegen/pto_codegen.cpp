...
std::string PTOCodegen::Generate(const ProgramPtr& program) {
  ...
  std::vector<FunctionPtr> kernel_functions;
  std::vector<FunctionPtr> orchestration_functions;

  for (const auto& [gvar, func] : program->functions_) {
    if (IsOrchestrationFunction(func)) {
  ...
  oss << "// Program: " << program->name_ << "\n";
  oss << "// Generated by PyPTO IR Compiler\n\n";

  // Generate kernel functions (PTO assembly)
  if (!kernel_functions.empty()) {
    oss << "// ========== Kernel Functions (PTO Assembly) ==========\n\n";
  ...
  // Generate orchestration functions (C++ Runtime code)
  if (!orchestration_functions.empty()) {
    oss << "// ========== Orchestration Functions (C++ Runtime) ==========\n\n";
    for (const auto& func : orchestration_functions) {
      std::string orch_code = GenerateOrchestrationCode(func);
      oss << orch_code << "\n";
    }
  }
...
std::string PTOCodegen::ResolvePhysicalTile(const std::string& var_name) {
  // Check if this variable has a mapping to a physical tile
  if (var_to_physical_tile_.count(var_name)) {
    return var_to_physical_tile_[var_name];
  }
...
bool PTOCodegen::IsOrchestrationFunction(const FunctionPtr& func) {
  // Orchestration functions are characterized by:
  // 1. Having TensorType parameters (representing DDR tensors)
  // 2. Containing Call operations to other functions (creating tasks)

  // Check if function has TensorType parameters
  bool has_tensor_params = false;
  for (const auto& param : func->params_) {
  ...
  return has_tensor_params;
}

std::string PTOCodegen::GenerateOrchestrationCode(const FunctionPtr& func) {
  // TODO: Implement full orchestration code generation
  // For now, return a placeholder message
  std::ostringstream oss;
  oss << "// Orchestration function: " << func->name_ << "\n";
  oss << "// TODO: Implement orchestration code generation\n";
  oss << "// This will generate C++ code with Runtime API:\n";
  oss << "//   int Build" << func->name_ << "(Runtime* runtime, uint64_t* args, int arg_count)\n\n";
  return oss.str();
}
