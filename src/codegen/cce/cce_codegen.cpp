...
CCECodegen::CCECodegen() : backend_(backend::GetBackendInstance(backend::BackendType::CCE)) {}
...
std::map<std::string, std::string> CCECodegen::Generate(const ir::ProgramPtr& program) {
  ...
  for (const auto& func : kernel_functions) {
    ...
    auto core_type = InferFunctionCoreType(func, backend::BackendType::CCE);
  ...
  if (!orchestration_functions.empty()) {
    ...
    auto orch_result = GenerateOrchestration(program, func, backend::BackendType::CCE);
