...
class PagedAttentionProgram:
    ...
    def kernel_qk_matmul(
        self,
        qi: pl.Tensor[[16, 16], pl.BF16],
        kj: pl.Tensor[[16, 16], pl.BF16],
        ...
        out: pl.Tensor[[16, 16], pl.FP32] = pl.l0c_store(result, [0, 0], [16, 16], output)
        return out
    ...
    def kernel_softmax_prepare(
        ...
        out_pij: pl.Tensor[[16, 16], pl.BF16],
        ...
        tmp_mi: pl.Tile[[16], pl.FP32] = pl.load(out_mi, [0], [16])
        ...
        tmp_li: pl.Tile[[16], pl.FP32] = pl.load(out_li, [0], [16])
        li_tile: pl.Tile[[16], pl.FP32] = pl.row_sum(exp_tile, tmp_li)
        pij_out: pl.Tensor[[16, 16], pl.BF16] = pl.store(exp_tile, [0, 0], [16, 16], out_pij)
        mi_out: pl.Tensor[[16], pl.FP32] = pl.store(mi_tile, [0], [16], out_mi)
        li_out: pl.Tensor[[16], pl.FP32] = pl.store(li_tile, [0], [16], out_li)
        return pij_out, mi_out, li_out

    # ── CUBE kernel: PV matmul ──────────────────────────────────────────
    @pl.function(type=pl.FunctionType.InCore)
    def kernel_pv_matmul(
        self,
        pij: pl.Tensor[[16, 16], pl.BF16],
        vj: pl.Tensor[[16, 16], pl.BF16],
        output: pl.Tensor[[16, 16], pl.FP32],
    ) -> pl.Tensor[[16, 16], pl.FP32]:
    ...
    def kernel_online_update(
        ...
        oi_r: pl.Tensor[[16, 16], pl.FP32] = pl.store(oi_tile, [0, 0], [16, 16], oi)
        out_r: pl.Tensor[[16, 16], pl.FP32] = pl.store(out_tile, [0, 0], [16, 16], out_view)
    ...
    def kernel_init_inplace(
        ...
        oi_tile: pl.Tile[[16, 16], pl.FP32] = pl.load(oi, [0, 0], [16, 16])
        li_tile: pl.Tile[[16], pl.FP32] = pl.load(li, [0], [16])
        mi_tile: pl.Tile[[16], pl.FP32] = pl.load(mi, [0], [16])
        oi_out: pl.Tensor[[16, 16], pl.FP32] = pl.store(oi_tile, [0, 0], [16, 16], oi)
        li_out: pl.Tensor[[16], pl.FP32] = pl.store(li_tile, [0], [16], li)
        mi_out: pl.Tensor[[16], pl.FP32] = pl.store(mi_tile, [0], [16], mi)
        return oi_out, li_out, mi_out
            ...
            for q_idx in pl.range(q_loop):
                ...
                for bn in pl.range(bn_this_batch):
                    ...
                    kj: pl.Tensor[[valid_len, head_dim], pl.BF16] = pl.view(
                        key_cache, [valid_len, head_dim], [cur_block_idx * block_size, 0]
                    )
                    vj: pl.Tensor[[valid_len, head_dim], pl.BF16] = pl.view(
                        value_cache, [valid_len, head_dim], [cur_block_idx * block_size, 0]
                    )

                    # QK matmul (CUBE)
                    sij: pl.Tensor[[q_tile, valid_len], pl.FP32] = self.kernel_qk_matmul(qi, kj)

                    # Softmax prepare (VECTOR) — outputs are per-block mi/li, not the inplace accumulators
                    pij, mi, li = self.kernel_softmax_prepare(sij, 1.0)  # type: ignore[reportArgumentType]

                    # PV matmul (CUBE)
                    oi_tmp: pl.Tensor[[q_tile, head_dim], pl.FP32] = self.kernel_pv_matmul(pij, vj)
                    ...
                    out_view: pl.Tensor[[q_tile, head_dim], pl.FP32] = pl.view(
                        out, [q_tile, head_dim], [cur_offset, 0]
                    )
                    mi_update, li_update, oi, _final = self.kernel_online_update(
