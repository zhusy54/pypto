...
def build_kernel_add(ib: IRBuilder, dtype: DataType = DataType.FP32):
    ...
    Adds two tiles element-wise: result = a + b
    ...
    with ib.function("kernel_add", type=ir.FunctionType.InCore) as f:
        # Parameters - TileType for InCore functions
        a = f.param("a", ir.TileType([16, 16], dtype))
        b = f.param("b", ir.TileType([16, 16], dtype))
        f.return_type(ir.TileType([16, 16], dtype))
...
def build_kernel_add_scalar(ib: IRBuilder, dtype: DataType = DataType.FP32):
    ...
    Adds a scalar to each element of a tile: result = a + scalar
    ...
    with ib.function("kernel_add_scalar", type=ir.FunctionType.InCore) as f:
        # Parameters - TileType and ScalarType
        a = f.param("a", ir.TileType([16, 16], dtype))
        scalar = f.param("scalar", ir.ScalarType(dtype))
        f.return_type(ir.TileType([16, 16], dtype))

        # Tile + scalar using block.adds
        result = ib.let("result", ir.op.block.adds(a, scalar))
...
def build_kernel_mul(ib: IRBuilder, dtype: DataType = DataType.FP32):
    ...
    Multiplies two tiles element-wise: result = a * b
...
def build_example_graph(ib: IRBuilder, dtype: DataType = DataType.FP32):
    ...
    with ib.function("BuildExampleGraph", type=ir.FunctionType.Orchestration) as f:
        # Parameters - TensorType for orchestration functions
        a = f.param("a", ir.TensorType([128, 128], dtype))
        b = f.param("b", ir.TensorType([128, 128], dtype))
        ...
        # Task 0: c = a + b (call kernel_add)
        kernel_add_op = ir.GlobalVar("kernel_add")
        c = ib.let("c", ir.Call(kernel_add_op, [a, b], ir.Span.unknown()))

        # Task 1: d = c + 1 (call kernel_add_scalar)
        kernel_add_scalar_op = ir.GlobalVar("kernel_add_scalar")
        d = ib.let("d", ir.Call(kernel_add_scalar_op, [c, scalar_1], ir.Span.unknown()))

        # Task 2: e = c + 2 (call kernel_add_scalar)
        e = ib.let("e", ir.Call(kernel_add_scalar_op, [c, scalar_2], ir.Span.unknown()))

        # Task 3: f = d * e (call kernel_mul)
        kernel_mul_op = ir.GlobalVar("kernel_mul")
        f_result = ib.let("f", ir.Call(kernel_mul_op, [d, e], ir.Span.unknown()))
...
def main():
    ...
    output_dir = ir.compile(program, strategy=ir.OptimizationStrategy.XPlatform, dump_passes=True)
