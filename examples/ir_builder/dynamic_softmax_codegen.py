...
def BuildDynamicSoftmaxFunction(ib: IRBuilder, dtype: DataType = DataType.FP32):
    ...
    with ib.function("dynamic_softmax") as f:
        ...
        # Scalar declarations
        tile_rows = ib.let("tile_rows", ir.ConstInt(8, DataType.INT32, ir.Span.unknown()))
        ...
        with ib.if_stmt(has_tail) as if_builder:
            ...
            tail_rowexpandsub_call = ir.Call(
                tail_rowexpandsub_op, [input_tensor, tail_temp_rowmax_updated, temp_shifted], ir.Span.unknown()
            )
            tail_temp_shifted_updated = ib.let("tail_temp_shifted_updated", tail_rowexpandsub_call)
            
            # 3. Call elem_exp - returns updated temp_exp
            tail_elem_exp_op = ir.GlobalVar("elem_exp")
            tail_elem_exp_call = ir.Call(tail_elem_exp_op, [tail_temp_shifted_updated, temp_exp], ir.Span.unknown())
            tail_temp_exp_updated = ib.let("tail_temp_exp_updated", tail_elem_exp_call)
            
            # 4. Call rowsum - returns updated temp_rowsum
            tail_rowsum_op = ir.GlobalVar("rowsum")
            tail_rowsum_call = ir.Call(tail_rowsum_op, [tail_temp_exp_updated, temp_rowsum], ir.Span.unknown())
            ...
            tail_rowexpanddiv_call = ir.Call(
                tail_rowexpanddiv_op, [tail_temp_exp_updated, tail_temp_rowsum_updated, output_after_loop], ir.Span.unknown()
            )
...
def main():
    ...
    print("=" * 70)
    print("\nSummary:")
    print(f"  - Module name: dynamic_softmax_module")
